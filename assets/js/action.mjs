...
/**
 * Manages the state and behavior of the machine web-application.
 */
class MachineApp {
  /**
   * @param {HTMLElement} configElement The DIV holding all the data-attributes.
   */
  constructor(configElement) {
    if (!configElement) {
      console.error('MachineApp cannot work without config.');
      return;
    }
  ...
  /**
   * Parse all settings from the config element and URL query parameters.
   */
  _parseSettings() {
    this.settings = {
  ...
  _applyQueryParameters() {
    ...
    for (const [key, value] of queryParams.entries()) {
      if (['temperature', 'frequency_penalty', 'presence_penalty', 'top_p'].includes(key)) {
